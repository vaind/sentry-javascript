...
function createEnvironment({ baseEnvironment } = {}) {
  ...
  return class SentryEnvironment extends BaseEnvironment {
    constructor(...args) {
      ...
      this.options = config.projectConfig.testEnvironmentOptions && config.projectConfig.testEnvironmentOptions.sentryConfig
      ...
      if (name.includes('start')) {
        ...
        if (spanProps.op === 'test') {
          const testTransaction = this.Sentry.startTransaction({
            ...
            tags: this.options.transactionOptions && this.options.transactionOptions.tags,
          })
